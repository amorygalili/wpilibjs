/**
 * Minimal OutlineViewer Compatible Server
 *
...
// Create a log file
const logStream = fs.createWriteStream('outlineviewer-minimal-server.log', { flags: 'a' });
...
function sendAllTopics(ws) {
  ...
  // Send all topics in a single message
  ...
  if (announceMessages.length > 0) {
    ws.send(JSON.stringify(announceMessages));
    log(`Sent ${announceMessages.length} topics to client`);
  }
...
function sendValue(ws, id, type, value, timestamp) {
  // Create value message - OutlineViewer expects a direct object, not an array
  const valueMessage = {
    id,
    timestamp,
    type,
  ...
  if (!topicName.includes('counter')) {
    log(`Sent value message for ${topicName}: ${JSON.stringify(valueMessage)}`);
  }
}

// Start the server
log('Minimal OutlineViewer Compatible Server started on port 5810');
