...
// Define custom types for WebSocket compatibility
type WebSocketType = WebSocketModule.WebSocket;

// Define MessageEvent if not available in the environment
...
// Define CloseEvent if not available in the environment
...
export class NT4_Client {
  ...
  private rttWs: WebSocketType | null = null;
  ...
  private async connectOnAlive() {
    ...
    try {
      result = await fetch(
        ...
        {
          signal: AbortSignal.timeout(250) as any,
        },
    ...
    if (result === null || !result.ok) {
      const requestLength = new Date().getTime() - requestStart;
  ...
  disconnect() {
    if (this.serverConnectionRequested) {
      ...
      if (this.serverConnectionActive && this.ws) {
        this.ws_onClose({ type: 'close', code: 1000, reason: 'Disconnect called', wasClean: true }, this.ws);
      }
  ...
  addTimestampedSample(topic: string, timestamp: number, value: any) {
    ...
    const txData = this.msgpackEncoder.encode([
      topicObj.uid,
      timestamp,
      topicObj.getTypeIdx(),
      value,
  ...
  private ws_sendJSON(method: string, params: any) {
    if (this.ws && this.ws.readyState === WebSocketModule.WebSocket.OPEN) {
      this.ws.send(
        JSON.stringify([
          {
            method: method,
  ...
  private ws_sendBinary(data: Uint8Array, rttWs = false) {
    const ws = rttWs ? this.rttWs : this.ws;
    if (ws && ws.readyState === WebSocketModule.WebSocket.OPEN) {
      ws.send(data);
    }
  }
  ...
  private ws_onOpen(ws: WebSocketType) {
    ...
    if (ws.protocol !== 'rtt.networktables.first.wpi.edu') {
      ...
      for (const topic of Array.from(this.publishedTopics.values())) {
  ...
  private ws_onClose(event: CloseEvent, source: WebSocketType) {
  ...
  private ws_onError(source: WebSocketType) {
    if (source !== this.ws) return;
    this.ws?.close();
    this.rttWs?.close();
  }
  ...
  private ws_onMessage(event: MessageEvent, rttOnly: boolean) {
    ...
    if (typeof event.data === 'string') {
      ...
      if (rttOnly) {
        console.warn(
          '[NT4] Ignoring text message, received by RTT only connection.',
        );
      }
      ...
      // MSGPack
      for (const unpackedData of Array.from(this.msgpackDecoder.decodeMulti(event.data))) {
        const topicID = (unpackedData as unknown[])[0] as number;
        const timestamp_us = (unpackedData as unknown[])[1] as number;
        const typeIdx = (unpackedData as unknown[])[2] as number;
        const value = (unpackedData as unknown[])[3];
        ...
        if (typeof typeIdx !== 'number') {
          console.warn(
            '[NT4] Ignoring binary data, type index is not a number',
          );
          return;
        }
        ...
        if (topicID >= 0) {
          ...
          for (const serverTopic of Array.from(this.serverTopics.values())) {
  ...
  private ws_resetTimeout() {
    ...
    const timeout =
      this.rttWs === null ? this.TIMEOUT_MS_V40 : this.TIMEOUT_MS_V41;
    this.disconnectTimeout = setTimeout(() => {
      console.log('[NT4] No data for ' + timeout.toString() + 'ms, closing');
      if (this.ws) {
        this.ws_onClose({ type: 'close', code: 1000, reason: 'Timeout', wasClean: false }, this.ws);
      }
    }, timeout);
  }
  ...
  private ws_connect(rttWs = false) {
    ...
    const ws = new WebSocketModule.WebSocket(
      this.serverAddr,
      rttWs
    ...
    ws.on('message', (data: any) => {
      const event = { data, type: 'message', target: ws } as MessageEvent;
    ...
    if (!rttWs) {
      ...
      ws.on('close', (code: number, reason: string) => {
        const event = { code, reason, wasClean: code === 1000, type: 'close' } as CloseEvent;
