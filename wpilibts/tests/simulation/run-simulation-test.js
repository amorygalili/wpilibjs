/**
 * Script to run the simulation test.
 *
 * This script:
 * 1. Builds the project if needed
 * 2. Runs the SimulationTest robot
 * 3. Waits for the tests to complete
...
// Check if port 1738 is already in use (NetworkTables)
function checkPortAndRun() {
  const tester = net.createServer()
    .once('error', err => {
      if (err.code === 'EADDRINUSE') {
        console.log('NetworkTables server is already running on port 1738.');
...
function runTest() {
  ...
  const tsNode = spawn('npx', ['ts-node', path.join(__dirname, 'SimulationTest.ts')], {
    stdio: 'inherit',
    shell: true
  });
  ...
  tsNode.on('close', code => {
    ...
    if (code === 0) {
      ...
      console.error(`Simulation test failed with code ${code}`);
    }
    process.exit(code);
  });
}
